...
if (process.env.NODE_ENV === "development") {
  app.get("*", (req, res) => {
    res.send(
      `<p>Development server is running!  Open <a href="http://localhost:3000">http://localhost:3000</a> instead.</p>
      <p>If you're seeing this in production, make sure NODE_ENV variable is set to "production"</p>`
    );
  });
} else {
  // Making a monolith by starting all services from the same file unless runnning in development.
  // This is for ease of deployment, though this could be broken up for more scalability later.
  startPythonMicroservice(
    "gunicorn",
    ["-w", "4", "-b", "0.0.0.0:6000", "app:app"],
    path.join(__dirname, "../microservices/calc_service")
  );
  startPythonMicroservice(
    "uvicorn",
    ["app:app", "--reload", "--port", "7000"],
    path.join(__dirname, "../microservices/chat_service")
  );
